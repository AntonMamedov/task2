...
int vec_push(void* elem, Vector *vec) {
    if (vec == NULL || elem == NULL)
        ...
        if (vec->alloc_size == 0) {
            ...
            vec->array = malloc(sizeof(void *) * DEFAULT_ALLOC_SIZE);
            if (vec->array == NULL)
                return -1;
            else {
            ...
            if (vec->size < vec->alloc_size)
                ...
                void **temp_vec = realloc(vec->array, sizeof(void *) * vec->alloc_size * VECTOR_EXTEND_CONST);
                if (temp_vec == NULL)
                    return -1;
                else {
...
void vec_release(void (*releaser)(void*), Vector *vec) {
    if (releaser != NULL){
